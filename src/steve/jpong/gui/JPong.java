...
public class JPong extends JFrame implements KeyListener{
	...
	public void animate(){
		...
		while(true){
			if(touching_vert_edge(InPlay)){
				vel_y = -vel_y;
				System.out.println("Touched vertical edge");
			}
			if(touching_horiz_edge(InPlay) && !has_touched){
				vel_x = -vel_x;
				System.out.println("Touching horizontal edge");
			}
	...
	public void keyPressed(KeyEvent evt){
		...
		case KeyEvent.VK_UP:
			gc.getPlayers().get(0).getPaddle().setVelocityY(3);
			break;
		case KeyEvent.VK_DOWN:
			gc.getPlayers().get(0).getPaddle().setVelocityY(-3);
			break;
		case KeyEvent.VK_Q:
			gc.getPlayers().get(1).getPaddle().setVelocityY(3);
			break;
		case KeyEvent.VK_A:
			gc.getPlayers().get(1).getPaddle().setVelocityY(-3);
			break;
	...
	public void keyReleased(KeyEvent evt){
		switch( evt.getKeyCode() ){
		case KeyEvent.VK_UP:
			gc.getPlayers().get(0).getPaddle().setVelocityY(0);
			break;
		case KeyEvent.VK_DOWN:
			gc.getPlayers().get(0).getPaddle().setVelocityY(0);
			break;
		case KeyEvent.VK_Q:
			gc.getPlayers().get(1).getPaddle().setVelocityY(0);
			break;
		case KeyEvent.VK_A:
			gc.getPlayers().get(1).getPaddle().setVelocityY(0);
			break;
